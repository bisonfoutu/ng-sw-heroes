...
4. Pour éviter des erreurs au démarrage de l'application, encapsulez ces éléments dans un `<ng-container>` disposant d'une directive `ngIf` qui affiche le contenu seulement si la longueur du tableau `messages` du `messageService` est positive.
