...
`onSelect()` est une méthode du HeroesComponent que vous allez écrire dans quelques instants.
Elle prend pour paramètre l'objet `hero` qui est affiché dans la balise `<li>`, celui-là même qui a été défini dans la directive `*ngFor`.
...
Maintenant, cliquez sur un des éléments de la liste. L'application semble fonctionner de nouveau.
Les héros apparaissent dans la liste et les détails du héros sélectionné sont affichés en bas de page, comme prévu.
